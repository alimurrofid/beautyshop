                            ...
                            <tbody>
                                ...
                                @foreach ($users as $user)
                                    <tr>
                                        ...
                                        <td
                                            class="bg-transparent border-b align-center">
                                            @if ($user->status == 0)
                                                <form action="{{ route('user.verif', $user->id) }}" method="POST"
                                                    id="verif-form-{{ $user->id }}">
                                                    @csrf
                                                    <button type="button" onclick="confirmVerif({{ $user->id }})"
                                                        data-tooltip-target="tooltip-sudah{{ $user->id }}"
                                                        class="bg-gradient-to-tl from-yellow-500 to-yellow-300 px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white">Unverified</button>
        ...
        function confirmVerif(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('verif-form-' + id).submit();
                    Swal.fire(
                        'Diverifikasi!',
